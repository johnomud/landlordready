...
function sendEmail(apiKey, to, subject, html) {
  return new Promise(function(resolve, reject) {
    var body = JSON.stringify({
      from: 'LandlordReady <noreply@landlordready.uk>',
      to: [to],
      subject: subject,
      html: html
    });
...
function packDownloadEmail(name, siteUrl, sessionId) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="margin:0;padding:0;background:#f8f6f1;font-family:'Plus Jakarta Sans',Arial,sans-serif;">
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f8f6f1;padding:40px 0;">
<tr><td align="center">
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(13,27,42,0.08);">
<tr><td style="background:#0d1b2a;padding:32px 40px;">
  <p style="margin:0;font-size:20px;font-weight:700;color:#ffffff;">Landlord<span style="color:#c97d0a;">Ready</span></p>
</td></tr>
<tr><td style="padding:40px;">
  <h1 style="margin:0 0 8px;font-size:24px;color:#0d1b2a;">Your document pack is ready, ${name || 'there'}.</h1>
  <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.6;">
    Thank you for your purchase. Your LandlordReady Renters&#8217; Rights Act 2025 Document Pack is ready to download.
  </p>
  <div style="background:#f8f6f1;border-radius:8px;padding:20px;margin-bottom:24px;">
    <p style="margin:0 0 8px;font-weight:700;color:#0d1b2a;font-size:14px;">Your pack includes:</p>
  ...
  <a href="${downloadUrl}" style="display:inline-block;background:#c97d0a;color:#0d1b2a;font-weight:700;font-size:15px;padding:14px 28px;border-radius:8px;text-decoration:none;margin-bottom:16px;">
  ...
  <div style="border-left:3px solid #c97d0a;padding:12px 16px;background:#f5e9cc;border-radius:0 8px 8px 0;margin-bottom:24px;">
  ...
  <p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">
    LandlordReady &#183; landlordready.uk<br>
...
<tr><td style="padding:40px;">
  <h1 style="margin:0 0 8px;font-size:24px;color:#0d1b2a;">Welcome to the Certificate Tracker, ${name || 'there'}.</h1>
  <p style="margin:0 0 24px;color:#6b7280;font-size:15px;line-height:1.6;">
  ...
  <a href="${trackerUrl}" style="display:inline-block;background:#c97d0a;color:#0d1b2a;font-weight:700;font-size:15px;padding:14px 28px;border-radius:8px;text-decoration:none;margin-bottom:24px;">
    Open Your Tracker &rarr;
  </a>
  <div style="background:#f8f6f1;border-radius:8px;padding:20px;margin-bottom:24px;">
    <p style="margin:0 0 8px;font-weight:700;color:#0d1b2a;font-size:14px;">What the tracker monitors:</p>
    <ul style="margin:0;padding-left:20px;color:#374151;font-size:14px;line-height:1.8;">
  ...
  <hr style="border:none;border-top:1px solid #e5e7eb;margin:24px 0;">
  <p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">
    LandlordReady &#183; landlordready.uk &#183; &#163;7/month &#8212; cancel anytime from your account.<br>
...
module.exports = async function(req, res) {
  ...
  var siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://landlordready.uk';
    ...
    if (resendKey && email) {
      try {
        if (product === 'pack') {
          ...
          await sendEmail(resendKey, email, 'Your LandlordReady Document Pack is ready', html);
          ...
          await sendEmail(resendKey, email, 'Welcome to LandlordReady Certificate Tracker', welcomeHtml);
